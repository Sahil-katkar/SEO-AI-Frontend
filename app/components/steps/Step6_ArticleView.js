...
export default function Step6_ArticleView() {
  ...
  const handleGenerateArticle = async () => {
    ...
    let mockArticle = `<h1>${projectData.outline.split('\n')[0].replace('H1: ','') || 'Generated Article Title'}</h1>\n\n`;
    mockArticle += `<p>This article discusses ${projectData.primaryKeyword || 'the topic'} targeting a ${projectData.targetAudience || 'general'} audience with an ${projectData.primaryIntent || 'informational'} intent. The desired tone is ${projectData.toneOfVoice || 'neutral'}.</p>\n\n`;
    
    const outlineLines = projectData.outline.split('\n').slice(1); // Skip H1
    outlineLines.forEach(line => {
        if (line.startsWith('H2:')) {
            mockArticle += `<h2>${line.replace('H2: ','')}</h2>\n<p>Detailed content for ${line.replace('H2: ','').toLowerCase()}...</p>\n`;
        } else if (line.startsWith('  H3:')) {
             mockArticle += `<h3>${line.replace('  H3: ','')}</h3>\n<p>Specific points about ${line.replace('  H3: ','').toLowerCase()}...</p>\n`;
        }
    });
  ...
  const handleDownload = (format = 'txt') => {
    ...
    const blob = new Blob([textContent], { type: `text/${format === 'md' ? 'markdown' : 'plain'};charset=utf-8` });
    ...
    <div className="step-component">
      ...
      {projectData.generatedArticle && !isLoading && (
        <div style={{marginTop: '20px'}}>
          ...
          <div
            style={{ border: '1px solid #ddd', padding: '15px', borderRadius: '4px', background: '#f9f9f9', minHeight: '200px' }}
            dangerouslySetInnerHTML={{ __html: projectData.generatedArticle }}
          ></div>
          <button onClick={handleCopyToClipboard} className="secondary" style={{marginTop: '15px'}}>Copy to Clipboard</button>
          <button onClick={() => handleDownload('txt')} className="secondary" style={{marginTop: '15px'}}>Download .txt</button>
          <button onClick={() => handleDownload('md')} className="secondary" style={{marginTop: '15px'}}>Download .md</button>
        </div>
      )}
    </div>
  );
